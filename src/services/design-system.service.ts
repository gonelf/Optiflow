...
export async function getDesignSystem(workspaceId: string) {
  ...
  return {
    ...
    colors: designSystem.colors as DesignTokens['colors'],
    typography: designSystem.typography as DesignTokens['typography'],
    spacing: designSystem.spacing as DesignTokens['spacing'],
    borderRadius: designSystem.borderRadius as DesignTokens['borderRadius'],
    shadows: designSystem.shadows as DesignTokens['shadows'],
    breakpoints: designSystem.breakpoints as DesignTokens['breakpoints'],
    customTokens: designSystem.customTokens,
  ...
  if (existingSystem) {
    return await prisma.designSystem.update({
      ...
      data: {
        ...
        borderRadius: mergedTokens.borderRadius,
        shadows: mergedTokens.shadows,
        breakpoints: mergedTokens.breakpoints,
      },
    });
  }
...
export async function updateTokenCategory<K extends keyof DesignTokens>(
  ...
  return await prisma.designSystem.upsert({
    ...
    create: {
      workspaceId,
      name: designSystem.name,
      colors: category === 'colors' ? tokens : designSystem.colors,
      typography: category === 'typography' ? tokens : designSystem.typography,
      spacing: category === 'spacing' ? tokens : designSystem.spacing,
      borderRadius: category === 'borderRadius' ? tokens : designSystem.borderRadius,
      shadows: category === 'shadows' ? tokens : designSystem.shadows,
      breakpoints: category === 'breakpoints' ? tokens : designSystem.breakpoints,
    },
    update: {
      [category]: tokens,
    },
